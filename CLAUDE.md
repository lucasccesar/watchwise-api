...
**Every custom `@Query` backing a `Page<T>` (or any query using `Pageable` for incremental, multi-call pagination) must `ORDER BY` a column set that's unique per row, not just `createdAt` alone** — a bug found across `Follower`, `FollowedPerson`, `Comment`, `DiaryEntry`, `DroppedEntry`, and `Like` (2026-09-06, `docs/pending/audit-completa-2026-09-04.md` items 7–8): ordering only by `createdAt` (non-unique — two rows can share the same millisecond under concurrent writes) leaves row order between two calls of the same paginated query up to Postgres' query plan, which can skip or duplicate rows across pages. The fix is always a compound `ORDER BY <column> DESC, <entity>.id DESC` (or `ASC`, matching the primary column's direction) — `id` is guaranteed unique, so it deterministically breaks every tie. Apply this to any new paginated `@Query` from the start; a derived-query method (`findByX(..., Pageable)`) with no explicit `@Query`/`ORDER BY` has the same problem in an even worse form (no ordering at all, not just a missing tie-breaker) and should be converted to an explicit `@Query` once pagination is added. This does not apply to a "top N" query where `Pageable` is only used to cap a single one-shot result (e.g. `DiaryEntryRepository.findTopByUserIdAndContentTypeOrderByCreatedAtDesc`, `WatchCompanionRepository.countGroupedByCompanionUserId*`) — those are documented in `business-rules.md` as deliberately untie-broken, since there's no second call to stay consistent with. `PageRequestFactory`'s own default (`PageRequest.of(page, size)` with no `Sort` when the caller doesn't pass `sortBy`, and a single-column `Sort` with no tie-breaker when it does) has the same gap for every service that calls the 2-arg or 4-arg `build(...)` overload without a service-level `@Query` supplying its own `ORDER BY` — known and tracked, not yet fixed (see `docs/pending/audit-completa-2026-09-04.md` item 8's "achado relacionado, fora de escopo").
