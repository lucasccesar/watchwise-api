...
public interface DiaryEntryRepository extends JpaRepository<DiaryEntry, UUID> {
    ...
    @Query(value = """
            ...
            series_agg AS (
                SELECT series_tmdb_id,
                       COUNT(DISTINCT (season_number, episode_number)) AS watched_episode_count,
            ...
            SELECT sa.series_tmdb_id AS seriesTmdbId,
                   sa.watched_episode_count AS watchedEpisodeCount,
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    List<SeasonProgress> findWatchedEpisodeProgressByUserIdAndSeriesTmdbIds(
            @Param("userId") UUID userId, @Param("seriesTmdbIds") Collection<String> seriesTmdbIds);
